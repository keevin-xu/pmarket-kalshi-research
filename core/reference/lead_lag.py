...
def detect_divergences(poly_series: Series, kalshi_series: Series, regime: str,
                       *, match_id: str = "", ll_params=None) -> list[Divergence]:
    """Confirmed divergences per the sport's frozen threshold + confirmation
    rule. As-of alignment; a run of >= confirmation_snapshots consecutive
    instants with |gap| >= threshold and CONSISTENT sign opens one divergence,
    then a one-window cooldown avoids re-counting. ll_params defaults to the
    generic LeadLagParams (== LoL's frozen values)."""
    ll_params = ll_params or LeadLagParams()
    thr = ll_params.divergence_threshold
    need = ll_params.confirmation_snapshots
    window = ll_params.convergence_window_s
...
def convergence_after(div: Divergence, poly_series: Series, kalshi_series: Series,
                      *, ll_params=None) -> float:
    ...
    w = (ll_params or LeadLagParams()).convergence_window_s
