...
def team_match(a: str, b: str, threshold: float | None = None) -> bool:
    ...
    threshold = 0.85 if threshold is None else threshold
...
def coverage_report(oe_matches: list[dict], venue_records: dict[str, list[dict]],
                    census_params) -> dict:
    """Compute cross-venue coverage against the sport's frozen census params.
    ...
    existence, and covered match ids.
    """
    tol = timedelta(minutes=census_params.coverage_join_tolerance_min)
    ...
    per_family: dict[str, int] = {f: 0 for f in census_params.families_phase1}
    ...
    return {
        ...
        "gate_min_covered": census_params.min_covered_matches,
        "passes_coverage": n_cov >= census_params.min_covered_matches,
    }
