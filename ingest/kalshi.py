...
from ingest.base import HTTP_HEADERS, SSL_CONTEXT, Adapter, VendorError, mid_or_none

# Phase-1 families -> Kalshi series tickers (from recon). Totals/props excluded.
SERIES_SERIES_WINNER = "KXLOL"       # League of Legends (series/match winner)
SERIES_MAP_WINNER = "KXLOLMAP"       # League of Legends Map Winner
PHASE1_SERIES = {SERIES_SERIES_WINNER: "series_winner", SERIES_MAP_WINNER: "map_winner"}
...
class KalshiAdapter(Adapter):
    ...
    def to_quote_rows(self, payload: dict) -> list[dict]:
        ...
        from db import store  # local import: keep store the single ts encoder
        return [{
    ...
    def to_contract_row(self, event: dict, market: dict) -> dict:
        """Map a Kalshi market to a `contracts` row. family from series."""
        series = market["ticker"].split("-")[0]
        family = PHASE1_SERIES.get(series)
        return {
            "contract_id": market["ticker"],
            "venue": self.venue,
            "family": family,
            "outcome_side": market.get("yes_sub_title"),
            "question_text": f'{event.get("title","")} — {market.get("yes_sub_title","")}',
            "rules_primary": market.get("rules_primary"),   # for G1 parity
            "status": market.get("status"),
            "result": market.get("result"),
            "open_time": event.get("markets", [{}])[0].get("open_time") or market.get("open_time"),
