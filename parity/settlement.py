...
def check_family_parity(oe_maps: list[dict], kalshi: list[dict],
                        polymarket: list[dict], family: str = "map_winner") -> ParityResult:
    """Aggregate same-claim parity vs the frozen rule. Inputs are per-map
    records {teams, ts, map_no, winner} from the sweeps (OE = neutral arbiter)."""
    ...
    min_n = CONFIG.parity.min_aligned_maps
    thr = CONFIG.parity.min_family_pass_rate
    if n_aligned < min_n:
