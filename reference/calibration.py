...
def calibration_report(points: Sequence[CalibrationPoint], venue: str, regime: str) -> dict:
    """Reliability curve + Brier/ECE/log-loss + bootstrap CI on Brier, for
    one (venue, regime). Returns a dict destined for a stored artifact."""
    ...
    curve = metrics.reliability_curve(prices, outcomes, CONFIG.reference.calibration_buckets)
...
def compare_venues(points: Sequence[CalibrationPoint], regime: str) -> dict:
    ...
    poly = calibration_report(points, CONFIG.venues.POLYMARKET, regime)
    kalshi = calibration_report(points, CONFIG.venues.KALSHI, regime)
    margin = CONFIG.reference.calibration_pass_margin
    passed = None
